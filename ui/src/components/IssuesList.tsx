        ...
        <div className="min-w-0 flex-1 space-y-2">
          <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-sm">
            <span className="font-medium text-foreground">
              {summary.doneCount}/{summary.totalCount} terminé(s)
            </span>
            <span className="text-muted-foreground">
              {summary.inProgressCount} en cours
            </span>
            <span className="text-muted-foreground">
              {summary.blockedCount} bloqué(s)
            </span>
            {showCostSummary && (
              <>
                <span
                  className="text-muted-foreground tabular-nums"
                  title={`${costSummary.runCount.toLocaleString()} exécution${
                    costSummary.runCount === 1 ? "" : "s"
                  } sur ${costSummary.issueCount} sous-ticket${
                    costSummary.issueCount === 1 ? "" : "s"
                  }`}
                >
                  {formatTokens(totalTokens)} jetons
                </span>
                <span className="text-muted-foreground tabular-nums">
                  {formatDurationMs(costSummary.runtimeMs)} durée
                </span>
              </>
            )}
          </div>
          <div
            role="progressbar"
            aria-label="Progression des sous-tickets"
            aria-valuemin={0}
          ...
          {target && targetIssue ? (
            <>
              <div className="text-xs font-medium text-muted-foreground">
                {target.kind === "next" ? "Prochain" : "En attente de bloqueurs"}
              </div>
              <Link
            ...
            <div className="text-sm font-medium text-foreground">Aucun sous-ticket actif</div>
          ) : summary.doneCount === summary.totalCount ? (
            <div className="text-sm font-medium text-foreground">Tous les sous-tickets sont terminés</div>
          ) : (
            <div className="text-sm font-medium text-foreground">Aucun sous-ticket actionnable</div>
          )}
        </div>
      </div>
    </div>
...
export function IssuesList({
  ...
  const groupedContent = useMemo(() => {
    ...
    if (viewState.groupBy === "workspace") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_workspace" ? "Sans espace de travail" : (workspaceNameMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "project") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_project" ? "Sans projet" : (projectById.get(key)?.name ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    if (viewState.groupBy === "parent") {
      ...
      return Object.keys(groups)
        ...
        .map((key) => ({
          key,
          label: key === "__no_parent" ? "Sans parent" : (issueTitleMap.get(key) ?? key.slice(0, 8)),
          items: groups[key]!,
        }));
    }
    // assignee
  ...
  const createActionLabel = createIssueLabel ? `Créer ${createIssueLabel}` : "Créer un ticket";
  const createButtonLabel = createIssueLabel ? `Nouveau ${createIssueLabel}` : "Nouveau ticket";
    ...
    <div ref={rootRef} className="space-y-4">
        ...
        <div className="flex items-center gap-0.5 sm:gap-1 shrink-0">
          ...
          {viewState.viewMode === "list" && (
            <Button
              ...
              title={viewState.nestingEnabled ? "Désactiver l'imbrication parent-enfant" : "Activer l'imbrication parent-enfant"}
            >
          ...
          {viewState.viewMode === "board" && (
            <>
              <Button
                ...
                title={boardCompactCards ? "Utiliser des cartes spacieuses" : "Utiliser des cartes compactes"}
              >
              ...
              <Button
                ...
                title={boardCollapsedStatuses.length > 0 ? "Développer les colonnes inactives" : "Réduire les colonnes inactives"}
              >
                ...
                <PopoverContent align="end" className="w-40 p-0">
                  <div className="p-2 space-y-0.5">
                    {KANBAN_COLUMN_PAGE_SIZE_OPTIONS.map((pageSize) => (
                      ...
                      >
                        <span>{pageSize} par colonne</span>
              ...
              <Button
                ...
                title="Réinitialiser la densité du tableau"
              >
          ...
          <IssueColumnPicker
            ...
            title="Choisir les colonnes de tickets visibles"
            iconOnly
          />
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title="Trier">
          ...
          {viewState.viewMode === "list" && (
            <Popover>
              <PopoverTrigger asChild>
                <Button variant="outline" size="icon" className="h-8 w-8 shrink-0" title="Grouper">
      ...
      {!searchWithinLoadedIssues && normalizedIssueSearch.length > 0 && searchedIssues.length === ISSUE_SEARCH_RESULT_LIMIT && (
        <p className="text-xs text-muted-foreground">
          Affichage de {ISSUE_SEARCH_RESULT_LIMIT} résultats maximum. Affinez la recherche pour réduire les résultats.
        </p>
      )}
      {boardColumnLimitReached && (
        <p className="text-xs text-muted-foreground">
          Certaines colonnes du tableau affichent jusqu'à {ISSUE_BOARD_COLUMN_RESULT_LIMIT} tickets. Affinez les filtres ou la recherche pour voir le reste.
        </p>
      )}
      {!isLoading && filtered.length === 0 && viewState.viewMode === "list" && (
        <EmptyState
          icon={CircleDot}
          message="Aucun ticket ne correspond aux filtres ou à la recherche actuels."
        ...
        <>
              ...
              <IssueGroupHeader
                ...
                trailing={(
                  <Button
                    ...
                    title={`Nouveau ticket dans ${group.label}`}
                    aria-label={`Nouveau ticket dans ${group.label}`}
                ...
                const renderIssueRow = (issue: Issue, depth: number) => {
                  ...
                  const visibleBlockerChips = unresolvedVisibleBlockers
                    .map((blockerId) => {
                      ...
                      const blockerStepSuffix = blockerStep ? ` \u00b7 \u00e9tape ${blockerStep}` : "";
                      return { blockerId, chipLabel: `bloqu\u00e9 par ${label}${blockerStepSuffix}` };
                    })
                  ...
                  const additionalVisibleBlockerLabel = additionalVisibleBlockerCount > 0
                    ? ` ... et ${additionalVisibleBlockerCount} de plus`
                    : "";
                          ...
                          <>
                            {hasChildren && !isExpanded ? (
                              <span className="ml-1.5 text-xs text-muted-foreground">
                                ({totalDescendants} sous-tâche{totalDescendants !== 1 ? "s" : ""})
                              </span>
                            ) : null}
                            {issueBadge ? (
                              issueBadge === "Paused" ? (
                            ...
                            {isSuccessfulRunHandoffRequired(issue) ? (
                              <span
                                ...
                                aria-label="Étape suivante requise"
                                title="Ce ticket nécessite une étape suivante"
                              >
          ...
          {(remainingIssueRowCount > 0 || hasMoreIssues || isLoadingMoreIssues) && (
            <div className="py-2" data-testid="issues-load-more-sentinel">
              <p className="text-xs text-muted-foreground">
                {isLoadingMoreIssues
                  ? "Chargement de tickets supplémentaires…"
                  : remainingIssueRowCount > 0
                    ? `Affichage de ${Math.min(renderedIssueRowLimit, filtered.length)} sur ${filtered.length} tickets`
                    : "Défiler pour charger plus de tickets"}
              </p>
            </div>
          )}
        </>
