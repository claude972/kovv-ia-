...
import { issueStatusIcon, issueStatusIconDefault, formatStatusLabel } from "../lib/status-colors";
...
function blockedAttentionLabel(blockerAttention: IssueBlockerAttention | null | undefined) {
  if (!blockerAttention || blockerAttention.state === "none") return "Bloqué";

  if (blockerAttention.reason === "active_child") {
    const count = blockerAttention.coveredBlockerCount;
    if (count === 1 && blockerAttention.sampleBlockerIdentifier) {
      return `Bloqué · en attente du sous-ticket actif ${blockerAttention.sampleBlockerIdentifier}`;
    }
    if (count === 1) return "Bloqué · en attente d'1 sous-ticket actif";
    return `Bloqué · en attente de ${count} sous-tickets actifs`;
  }

  if (blockerAttention.reason === "active_dependency") {
    const count = blockerAttention.coveredBlockerCount;
    if (count === 1 && blockerAttention.sampleBlockerIdentifier) {
      return `Bloqué · couvert par la dépendance active ${blockerAttention.sampleBlockerIdentifier}`;
    }
    if (count === 1) return "Bloqué · couvert par 1 dépendance active";
    return `Bloqué · couvert par ${count} dépendances actives`;
  }
  ...
  if (blockerAttention.reason === "stalled_review") {
    ...
    if (count === 1 && leaf) return `Bloqué · revue au point mort sur ${leaf}`;
    if (count === 1) return "Bloqué · revue au point mort, sans étape suivante claire";
    return `Bloqué · ${count} revues au point mort, sans étape suivante claire`;
  }

  if (blockerAttention.reason === "attention_required") {
    const count = blockerAttention.attentionBlockerCount || blockerAttention.unresolvedBlockerCount;
    const attentionCopy = `${count} ${count === 1 ? "bloqueur requiert" : "bloqueurs requièrent"} votre attention`;
    const coveredCount = blockerAttention.coveredBlockerCount;
    if (coveredCount > 0) {
      return `Bloqué · ${attentionCopy} ; ${coveredCount} couvert(s) par du travail actif`;
    }
    return `Bloqué · ${attentionCopy}`;
  }
...
export function StatusIcon({ status, blockerAttention, onChange, className, showLabel }: StatusIconProps) {
  ...
  const ariaLabel = status === "blocked" ? blockedAttentionLabel(blockerAttention) : formatStatusLabel(status);
  ...
  if (!onChange) return showLabel ? <span className="inline-flex items-center gap-1.5">{circle}<span className="text-sm">{formatStatusLabel(status)}</span></span> : circle;
