    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <MetricTile
          ...
          subtitle="Débits moins crédits sur la période sélectionnée"
          icon={ReceiptText}
        />
        <MetricTile
          label="Estimé"
          value={formatCents(estimatedDebitCents)}
          subtitle="Débits estimés non encore confirmés par facture"
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  if (!selectedCompanyId) {
    return <EmptyState icon={DollarSign} message="Sélectionnez une entreprise pour consulter les coûts." />;
  }
    ...
    <div className="space-y-6">
            ...
            <MetricTile
              ...
              subtitle={
                activeBudgetIncidents.length > 0
                  ? `${budgetData?.pausedAgentCount ?? 0} agents en pause · ${budgetData?.pausedProjectCount ?? 0} projets en pause`
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
      ...
      <Tabs value={mainTab} onValueChange={(value) => setMainTab(value as typeof mainTab)}>
        <TabsList variant="line" className="justify-start">
          ...
          <TabsTrigger value="billers">Facturiers</TabsTrigger>
                    ...
                    {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0 ? (
                      <div className="space-y-2">
                        ...
                        <div className="text-xs text-muted-foreground">
                          {spendData.summary.utilizationPercent}% du budget mensuel consommé sur cette période.
                        </div>
                      </div>
                    ) : null}
                  </CardContent>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.25fr,0.95fr]">
                <Card>
                  <CardHeader className="px-5 pt-5 pb-2">
                    ...
                    <CardDescription>Ce que chaque agent a consommé sur la période sélectionnée.</CardDescription>
                  </CardHeader>
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  entrée {formatTokens(row.inputTokens + row.cachedInputTokens)} · sortie {formatTokens(row.outputTokens)}
                                </div>
                ...
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      ...
                      <CardDescription>Coûts des runs attribués via les tâches liées au projet.</CardDescription>
                    </CardHeader>
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">Aucun coût de run attribué à un projet pour l'instant.</p>
                      ) : (
                  ...
                  <FinanceTimelineCard rows={topFinanceEvents.slice(0, 6)} emptyMessage="Aucun événement financier pour l'instant. Ajoutez des charges au niveau du compte dès réception des factures ou crédits des facturiers." />
                </div>
              </div>
            </>
          )}
            ...
            <>
              <Card className="border-border/70 bg-[linear-gradient(180deg,rgba(255,255,255,0.05),rgba(255,255,255,0.02))]">
                <CardHeader className="px-5 pt-5 pb-3">
                  <CardTitle className="text-base">Plan de contrôle budgétaire</CardTitle>
                  <CardDescription>
                    Plafonds de dépenses fermes pour les agents et les projets. Le quota d'abonnement fournisseur est séparé et apparaît sous Fournisseurs.
                  </CardDescription>
                </CardHeader>
                <CardContent className="grid gap-3 px-5 pb-5 pt-0 md:grid-cols-4">
                  <MetricTile
                    ...
                    subtitle="Franchissements de seuil doux ou dur ouverts"
                    icon={ReceiptText}
                  />
                  <MetricTile
                    label="Approbations en attente"
                    value={String(budgetData?.pendingApprovalCount ?? 0)}
                    subtitle="Dérogations budgétaires en attente d'action du conseil"
                    icon={ArrowUpRight}
                  />
                  <MetricTile
                ...
                <div className="space-y-3">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">
                      Résolvez les blocages fermes ici en augmentant le budget ou en maintenant explicitement la portée en pause.
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold capitalize">
                          {scopeType === "company" ? "Budgets d'entreprise" : scopeType === "agent" ? "Budgets par agent" : "Budgets par projet"}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? "Politique mensuelle à l'échelle de l'entreprise."
                            : scopeType === "agent"
                              ? "Politiques de dépenses mensuelles récurrentes par agent."
                ...
                {budgetPolicies.length === 0 ? (
                  <Card>
                    <CardContent className="px-5 py-8 text-sm text-muted-foreground">
                      Aucune politique budgétaire pour l'instant. Définissez les budgets des agents et projets depuis leurs pages de détail, ou utilisez le contrôle budgétaire mensuel d'entreprise existant.
                    </CardContent>
                  </Card>
                ) : null}
              </div>
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucun événement de coût sur cette période.</p>
                  ) : (
          ...
          {showCustomPrompt ? (
            ...
            <>
              <Tabs value={effectiveBiller} onValueChange={setActiveBiller}>
                ...
                <TabsContent value="all" className="mt-4">
                  {billers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">Aucun événement facturable sur cette période.</p>
                  ) : (
                    <div className="grid gap-4 md:grid-cols-2">
                      {billers.map((biller) => {
            ...
            <>
              ...
              <div className="grid gap-4 xl:grid-cols-[1.2fr,0.95fr]">
                <div className="space-y-4">
                  <Card>
                    <CardHeader className="px-5 pt-5 pb-2">
                      <CardTitle className="text-base">Par facturier</CardTitle>
