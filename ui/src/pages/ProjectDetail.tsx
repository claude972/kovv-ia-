...
export function ProjectDetail() {
  ...
  const archiveProject = useMutation({
    ...
    onError: (_, archived) => {
      pushToast({
        title: archived ? "Impossible d'archiver le projet" : "Impossible de désarchiver le projet",
        tone: "error",
      });
    },
  });
  ...
  return (
    <div className="space-y-6">
      {showLeftProjectNotice ? (
        <div className="flex items-center gap-3 border border-yellow-300/35 bg-yellow-300/10 px-3 py-2 text-sm text-yellow-100">
          <p className="min-w-0 flex-1">
            Vous avez quitté ce projet. Il n'apparaît plus dans votre barre latérale.
          </p>
          <MembershipAction
            compact
            state="left"
          ...
          <button
            ...
            aria-label="Masquer la notification d'appartenance au projet"
        ...
        <div className="min-w-0 space-y-2">
          ...
          {project.pauseReason === "budget" ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-red-500/30 bg-red-500/10 px-3 py-1 text-[11px] font-medium uppercase tracking-[0.18em] text-red-200">
              ...
              Mis en pause par limite budgétaire stricte
            </div>
          ) : null}
          {project.managedByPlugin ? (
            <div className="inline-flex items-center gap-2 rounded-full border border-border bg-muted px-3 py-1 text-[11px] font-medium text-muted-foreground">
              ...
              Géré par {project.managedByPlugin.pluginDisplayName}
            </div>
          ) : null}
        </div>
      </div>
      ...
      <Tabs value={activeTab ?? "list"} onValueChange={(value) => handleTabChange(value as ProjectTab)}>
        <PageTabBar
          items={[
            { value: "list", label: "Tickets" },
            { value: "overview", label: "Aperçu" },
            ...(project.managedByPlugin ? [{ value: "plugin-operations", label: "Opérations du plugin" }] : []),
